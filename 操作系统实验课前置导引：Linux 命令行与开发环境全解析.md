...
一份合格的 Linux 上机实验报告应包含以下核心板块，请在撰写时保持排版整洁、逻辑清晰：

- **基本信息：** 包含实验名称、姓名、学号、班级以及实验日期。
- **实验目的：** 简明扼要地概括本次实验需要掌握的核心技能（如熟悉 Linux 目录操作、掌握 GCC 编译流程等）。
- **实验环境：** 准确记录你所使用的 Linux 发行版（如 Ubuntu 22.04、CentOS 等）以及 GCC 编译器的版本信息。
- **实验过程与实操记录（核心环节）：** * 不要大段抄写教程原文。
  - 必须结合你自己的操作过程，附带**关键步骤的终端全屏截图**（截图中需能清晰看到你的命令行提示符、输入的命令以及系统的回显输出）。
  - 截图下方需配有简短的文字说明，解释该条命令的作用。
- **实验总结与问题反馈：** 记录你在配置环境或敲击命令时遇到的报错（如拼写错误、权限不足、路径找不到等），以及你是如何通过 `man` 手册或查阅资料解决这些问题的。

### 2. 深度思考题 (必答环节)

请结合本实验指导书的内容以及你的实际上机体验，在实验报告的末尾详细解答以下三个模块的思考题。

#### 模块一：Linux 常用命令与文件系统探秘

- **路径与导航：** 假设你当前所在的绝对路径是 `/home/student/workspace/os_lab0`，请写出一条只使用**相对路径**的命令，让你一步切换到系统配置目录 `/etc` 下。
- **权限与安全：** 教程中提到给文件赋予 `755` 权限（即执行命令 `chmod 755 program`）。请拆解数字 `755`，分别说明文件的属主（拥有者）、同组用户以及其他用户具体获得了哪些权限（读、写、执行）？
- **安全防范意识：** 在 Linux 技术社区中，常有人开玩笑说执行 `rm -rf /*` 就能“跑路”。请结合本教程中对 `rm` 命令的讲解，详细解释这条指令为何具有毁灭性的破坏力。

#### 模块二：GCC 编译器的淬炼

- **编译选项辨析：** 在构建大型 C 语言工程时，我们经常看到 `gcc -c main.c` 这样的指令。请解释加上 `-c` 选项后，GCC 编译器的行为发生了什么具体变化？这与直接使用 `gcc main.c -o main` 有何本质区别？
- **代码规范与排错：** 为什么在这门操作系统实验课中，强烈建议大家在编译时永远带上 `-Wall` 选项？如果你忽略了系统发出的 Warning（警告）直接强行运行，可能会在后续内存或进程实验中埋下什么隐患？

#### 模块三：Make 工具与自动化构建的智慧

- **致命的语法陷阱：** 在编写 Makefile 文件时，有一条关乎整个构建系统能否正常运行的“铁律”。请明确指出，在编写具体执行命令（如 `gcc ...` 或 `rm ...`）的那一行，最前端必须使用什么符号进行缩进？绝不能用什么符号代替？
- **增量编译的原理：** 假设你的工程包含 `main.c` 和 `utils.c`，并且你已经通过 `make` 命令成功生成了可执行文件 `os_lab`。此时，如果你仅仅修改了 `utils.c` 里面的一个拼写错误并保存，再次在终端输入 `make`，系统会自动重新编译 `main.c` 吗？请结合 Makefile 的“依赖推导”机制，解释 Make 工具是如何判断哪些文件需要重新编译的。